...
$n = 1;
for ($i = 0; $i < $total; $i++) {
    $query = "INSERT INTO klasifikasi ( ipk, sks_lulus, penghasilan_orangtua, tanggungan, actual, predict) 
    VALUES (
    (SELECT ipk FROM testing WHERE id='$n'),
    (SELECT sks_lulus FROM testing WHERE id='$n'),
    (SELECT penghasilan_orangtua FROM testing WHERE id='$n'),
    (SELECT tanggungan FROM testing WHERE id='$n'),
    (SELECT status FROM testing WHERE id='$n'),'$test[$i]')";
